...
export default function Destination() {
  ...
  // Single effect for fetching + slug filtering
  useEffect(() => {
    const fetchData = async () => {
      try {
        const packagesRes = await axios.get(`${API_BASE}/tour-packages`);
        const filteredPackages = packagesRes.data.data.filter(
          (pkg: TourPackage) => pkg.status?.id === 4
        );

        const destinationsRes = await axios.get(`${API_BASE}/destinations`);
        const allDestinations = destinationsRes.data.data;
        ...
        const sortedDestinations = destinationsWithPackages.sort((a: Destination, b: Destination) =>
          a.name.localeCompare(b.name)
        );

        const finalDestinations = [{ id: 0, name: "All" }, ...sortedDestinations];
        setDestinations(finalDestinations);

        // Apply slug filter immediately
        if (slug) {
          const destination = finalDestinations.find(
            (dest) => createSlug(dest.name) === slug
          );
          if (destination) {
          ...
          setSelectedDestination(0); // default to "All"
        }
  ...
  const handleDestinationSelect = (destinationId: number | null) => {
    ...
    if (destinationId === null || destinationId === 0) {
      ...
      if (destination) {
        ...
        navigate(`/destination/${slug}`, { replace: true });
      } else {
        navigate("/destination", { replace: true });
      }
    }
  ...
  const getSelectedDestinationName = () => {
    if (!selectedDestination || selectedDestination === 0) return "All Destinations";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-6">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600 mt-1">
              Explore amazing tours in {getSelectedDestinationName()}
            </p>
          </div>
        </div>
      )}
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">
              Try adjusting your search or destination filters
            </p>
            <button
          ...
          <>
            ...
            <div className="mb-6 text-center">
              <p className="text-sm text-gray-600">
                ...
                {selectedDestination && selectedDestination !== 0 && ` in ${getSelectedDestinationName()}`}
              </p>
            </div>
